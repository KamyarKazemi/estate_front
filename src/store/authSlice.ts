...
import { sendNumberLoginThunk } from "./thunks/sendNumberLoginThunk";
import { sendOtpLoginThunk } from "./thunks/sendOtpLoginThunk";
...
const initialState: AuthState = {
  ...
  changePhoneOtpSessionToken: null,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(completeRegister.fulfilled, (state, action) => {
        ...
        state.user = user;
        persistStoredAuth({ accessToken, refreshToken, user });
      })
      ...
      .addCase(resetPasswordPhoneThunk.pending, (state) => {
        state.sendingResetPhone = true;
        state.error = null;
      })
      .addCase(resetPasswordPhoneThunk.fulfilled, (state, action) => {
        state.sendingResetPhone = false;
        state.resetOtpSessionToken = action.payload.otp_session_token;
      })
      ...
      .addCase(changePhoneThunk.pending, (state) => {
        state.sendingChangePhone = true;
        state.error = null;
      })
      .addCase(changePhoneThunk.fulfilled, (state, action) => {
        state.sendingChangePhone = false;
        state.changePhoneOtpSessionToken = action.payload.otp_session_token;
      })
      ...
      .addCase(changePhoneOtpThunk.pending, (state) => {
        state.verifyingChangePhoneOtp = true;
        state.error = null;
      })
      .addCase(changePhoneOtpThunk.fulfilled, (state, action) => {
        state.verifyingChangePhoneOtp = false;
        state.changePhoneOtpSessionToken = null;
        // update the phone number in user profile
        if (state.user) {
          state.user = {
            ...state.user,
