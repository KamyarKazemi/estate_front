...
export default function DesktopNav({
  ...
  const isSignedIn = useSelector((state: RootState) => !!state.auth.access_token);

  const baseLiClass = `relative cursor-pointer transition-colors duration-200 text-sm font-light tracking-wide`;
  ...
  return (
    <ul className="relative hidden gap-6 md:flex lg:gap-15">
      ...
      >
        ...
        >
          <span>آگهی ها</span>
          <span className={`text-xs transition-transform duration-200 ${isOpen ? "rotate-180" : "rotate-0"}`}>
        ...
        >
          <div className="flex flex-col gap-0.5 p-2.5">
            {["بخر", "اجاره کن", "نو ساز ها", "املاک لوکس"].map((label, i) => {
              const paths = ["/listings/buy", "/listings/rent", "/listings/new", "/listings/luxury"];
              return (
                <Link
                  key={label}
                  to={paths[i]}
      ...
      >
        ...
        <div
          className={`
            ...
            transition-all duration-200 ease-out
            ${hoveredIndex === 3 ? "pointer-events-auto translate-y-0 opacity-100" : "pointer-events-none -translate-y-2 opacity-0"}
          `}
        >
          {isSignedIn ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-slate-400">{user?.phone_number || user?.email || "حساب فعال"}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="rounded-xl bg-white/5 px-3 py-2">
                  ...
                  <p className="mt-1 truncate text-slate-200">{user?.email || "-"}</p>
                </div>
              </div>

              <div className="flex gap-2">
                <Link to="/dashboard" className="flex-1 rounded-xl bg-sky-500/15 px-3 py-2 text-center text-xs text-sky-100 transition hover:bg-sky-500/25">
                  داشبورد
                </Link>
                <button type="button" onClick={handleLogout} className="flex-1 rounded-xl bg-white/5 px-3 py-2 text-xs text-slate-200 transition hover:bg-white/10">
                  خروج
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-slate-400">برای دیدن داشبورد و اطلاعات حساب وارد شوید.</p>
              </div>
              <Link to="/profile" className="block rounded-xl bg-sky-500/15 px-3 py-2 text-center text-xs text-sky-100 transition hover:bg-sky-500/25">
                ورود / ثبت نام
              </Link>
            </div>
          )}
