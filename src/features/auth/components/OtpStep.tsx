    ...
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex justify-center gap-2">
        {Array.from({ length: 5 }).map((_, idx) => (
          <div key={idx} className="h-12 w-12 rounded-xl bg-white/5 sm:h-14 sm:w-14" />
        ))}
      </div>
...
export function OtpStep({
  otp, otpRefs, isComplete, loading, skeletonLoading = false,
  handleChange, handleKeyDown, handlePaste, onSubmit,
}: OtpStepProps) {
  ...
  if (skeletonLoading) return <OtpStepSkeleton />;
  ...
  return (
    ...
    >
      <div className="text-center">
        <p className="text-sm text-slate-400">کد تأییدی که برای شماره شما ارسال شده را وارد کنید</p>
      </div>
      ...
      <motion.button
        ...
        "
      >
        {loading ? (<><Spinner /><span>در حال تأیید کد...</span></>) : "تأیید کد"}
      </motion.button>
    </motion.form>
  );
}
