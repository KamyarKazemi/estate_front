...
export function StepNavigation({ steps, currentStep, completion, labels, onStepClick }: StepNavigationProps) {
  return (
      ...
      {steps.map((step, index) => {
        ...
        return (
          <div key={step} className="flex items-center flex-1">
            ...
            <span className={`ms-2.5 sm:ms-3 truncate text-[11px] sm:text-sm font-medium transition-colors duration-300 ${active ? "text-white" : "text-slate-400"}`}>
              {labels[step]}
            </span>
