...
export function PhoneStep({
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (digits.length === 1 && digits !== "0" && digits !== "9") return;
  ...
  if (skeletonLoading) return <PhoneStepSkeleton withPassword={isLogin} />;
    ...
    >
      ...
      {isLogin && (
        <>
          ...
          >
            <label className="mb-3 block text-sm font-medium text-white">رمز عبور</label>
            <div className="relative">
              ...
              <button
                ...
                tabIndex={-1}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 transition-colors duration-200 hover:text-sky-200"
              >
          ...
          <motion.div className="flex justify-center items-center w-full">
            <GlassButton onClick={() => navigate("/reset-password")}>فراموشی رمز</GlassButton>
          </motion.div>
        </>
      )}
      ...
      >
        {loading ? (
          <><Spinner /><span>در حال ارسال کد تأیید...</span></>
        ) : (
          "دریافت کد تأیید"
        )}
      </motion.button>
