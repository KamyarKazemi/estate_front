...
import { resetPasswordPhoneThunk } from "../../store/thunks/resetPasswordPhoneThunk";
import { resetPasswordOtpThunk } from "../../store/thunks/resetPasswordOtpThunk";
import { resetPasswordThunk } from "../../store/thunks/resetPasswordThunk";
import { resetPasswordFlow } from "../../store/authSlice";
...
function ResetPassword() {
  ...
  const maxAllowedStep: Step = resetPasswordToken ? 3 : resetOtpSessionToken ? 2 : 1;
  ...
  /* -------- step 2: otp -------- */
  const { otp, otpRefs, isComplete, handleChange, handleKeyDown, handlePaste } = useOtp(5);

  const handleOtpSubmit = async () => {
    if (!resetOtpSessionToken) return;
    try {
      await dispatch(
        resetPasswordOtpThunk({ otp_session_token: resetOtpSessionToken, otp_code: otp.join("") }),
      ).unwrap();
      setStep(3);
    } catch {
      // error lands in state.error via Redux
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      await dispatch(
        resetPasswordThunk({ reset_token: resetPasswordToken, password: newPassword, confirm_password: confirmPassword }),
      ).unwrap();
      navigate("/dashboard");
    } catch {
      // error lands in state.error via Redux
      ...
      <div className="relative z-10 w-full max-w-xl">
        ...
        >
          ...
          <StepNavigation
            ...
            labels={stepLabels}
            onStepClick={(target) => { if (target <= maxAllowedStep) setStep(target); }}
          />
