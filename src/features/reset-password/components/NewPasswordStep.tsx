...
import { passwordRegex } from "../../auth/hooks/useSignupValidation";
...
export function NewPasswordStep({ newPassword, setNewPassword, confirmPassword, setConfirmPassword, loading, onSubmit }: Props) {
  const [showNew, setShowNew] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);

  const isPasswordValid = passwordRegex.test(newPassword);
  const passwordsMatch = newPassword === confirmPassword;
  const isValid = isPasswordValid && passwordsMatch && confirmPassword.length > 0;
  ...
  return (
    ...
    >
      <div>
        <label className="mb-3 block text-sm font-medium text-white">رمز عبور جدید</label>
        ...
        {newPassword.length > 0 && !isPasswordValid && (
          <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="mt-2 text-sm text-red-400">
      ...
      <div>
        <label className="mb-3 block text-sm font-medium text-white">تکرار رمز عبور جدید</label>
        <div className="relative">
          <motion.input
      ...
      <motion.button
        ...
        "
      >
        {loading ? (<><Spinner /><span>در حال تغییر رمز...</span></>) : "تغییر رمز عبور"}
      </motion.button>
    </motion.form>
  );
}
